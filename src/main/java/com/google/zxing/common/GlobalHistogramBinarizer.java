...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        for (int i2 = 0; i2 < width; i2++) {
            int i3 = (row[i2] & DeviceInfos.NETWORK_TYPE_UNCONNECTED) >> 3;
            iArr[i3] = iArr[i3] + 1;
        }
        int estimateBlackPoint = estimateBlackPoint(iArr);
        if (width < 3) {
            for (int i4 = 0; i4 < width; i4++) {
                if ((row[i4] & DeviceInfos.NETWORK_TYPE_UNCONNECTED) < estimateBlackPoint) {
                    bitArray.set(i4);
                }
            }
        } else {
            byte b = row[1] & DeviceInfos.NETWORK_TYPE_UNCONNECTED;
            byte b2 = row[0] & DeviceInfos.NETWORK_TYPE_UNCONNECTED;
            int i5 = 1;
            while (i5 < width - 1) {
                int i6 = i5 + 1;
                byte b3 = row[i6] & DeviceInfos.NETWORK_TYPE_UNCONNECTED;
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            for (int i3 = width / 5; i3 < i2; i3++) {
                int i4 = (row[i3] & DeviceInfos.NETWORK_TYPE_UNCONNECTED) >> 3;
                iArr[i4] = iArr[i4] + 1;
            }
        }
        int estimateBlackPoint = estimateBlackPoint(iArr);
        ...
        for (int i5 = 0; i5 < height; i5++) {
            ...
            for (int i7 = 0; i7 < width; i7++) {
                if ((matrix[i6 + i7] & DeviceInfos.NETWORK_TYPE_UNCONNECTED) < estimateBlackPoint) {
                    bitMatrix.set(i7, i5);
                }
            }
        }
