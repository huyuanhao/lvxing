...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    /* renamed from: androidx.recyclerview.widget.MessageThreadUtil$MessageQueue */
    ...
    /* renamed from: androidx.recyclerview.widget.MessageThreadUtil$SyncQueueItem */
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem next = C04491.this.mQueue.next();
                    while (next != null) {
                        int i = next.what;
                        if (i == 1) {
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem next = C04512.this.mQueue.next();
                        if (next == null) {
                            C04512.this.mBackgroundRunning.set(false);
                            return;
                        }
                        int i = next.what;
                        if (i == 1) {
                            C04512.this.mQueue.removeMessages(1);
                            backgroundCallback.refresh(next.arg1);
                        } else if (i == 2) {
                            C04512.this.mQueue.removeMessages(2);
                            C04512.this.mQueue.removeMessages(3);
